...
def filter_by_date(df, date_column, label_prefix=""):
    ...
    # Now min() and max() will work correctly
    min_date = df[date_column].min().date()
    max_date = df[date_column].max().date()
    ...
    try:
        ...
        with chart_col1:
            ...
            if df_customers is not None and not df_customers.empty:
                ...
                # If Province Data is Empty, show title only once for missing data
...
def show_cj_page():
    try:
        ...
        if not filtered_df.empty and 'Event' in filtered_df.columns and 'Time_On_Page' in filtered_df.columns:
            ...
            # In case there's no data available, show message with titles for both columns
            col1, col2 = st.columns(2)
            with col1:
        ...
        # Column 1: Summary of Total Time Spent Per Product
        with chart_col1:
